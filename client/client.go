...
func GetFilePtr(userdata *User, File_PTR_uuid userlib.UUID) (res File_PTR, dk []byte, hkey []byte, err error) {
	...
	if !ok {
		return file_ptr, nil, nil, errors.New(strings.ToTitle("Trouble finding invitation struct..."))
	}
	...
	if !integrity {
		return file_ptr, nil, nil, errors.New(strings.ToTitle("User's mapval has been tampered with"))
	}
...
func CreateInvitationStruct(filename string, userdata *User, recipient string) (err error) {
	...
	invitation.File_UUID = File_PTR_uuid
	...
	userdata.Files_Owned[File_PTR_uuid] = ownMap

	UpdateUser(userdata)
	userlib.DatastoreSet(invitation_uuid, total_invitation)

	// shared_with := []string{}
	// 	userdata.Files_Owned[File_PTR_uuid] = shared_with
	return nil
}
...
func GetFilePtrMaster(userdata *User, filename string) (ptr File_PTR, dk []byte, hkey []byte, err error) {
	...
	var file_ptr File_PTR
	if err != nil {
		return file_ptr, nil, nil, errors.New(strings.ToTitle("troubling computing uuid!"))
	}
	_, okOwn := userdata.Files_Owned[File_PTR_uuid]
	_, okAccess := userdata.Files_Access[File_PTR_uuid]
	if !okOwn && !okAccess {
		return file_ptr, nil, nil, errors.New(strings.ToTitle("File not Found in Namespace!"))
	}
	...
	if okOwn {
		...
		hmac_key, err = userlib.HashKDF(userdata.MasterKey, []byte("hmac_key"+filename))
		hmac_key = hmac_key[:16]
		file_ptr, err = GetFilePtrOwner(File_PTR_uuid, file_decrypt_key, hmac_key)
...
func (userdata *User) LoadFile(filename string) (content []byte, err error) {
	...
	file_ptr, file_decrypt_key, hmac_key, err := GetFilePtrMaster(userdata, filename)
	//at this point, regardless of whether user is owner or just accessee, they have the file ptr
...
func (userdata *User) CreateInvitation(filename string, recipientUsername string) (invitationPtr uuid.UUID, err error) {
	...
	_, okOwn := userdata.Files_Owned[File_PTR_uuid]
	mapVal, okAccess := userdata.Files_Access[File_PTR_uuid]
	if !okOwn && !okAccess {
		...
		invitation_struct_uuid := userdata.Files_Owned[File_PTR_uuid].Invitation_UUID
...
func (userdata *User) RevokeAccess(filename string, recipientUsername string) error {
	...
	*/
	...
	ownMap, okOwn := userdata.Files_Owned[File_PTR_uuid]
	if !okOwn {
		return errors.New(strings.ToTitle("Trying to revoke access yet file isn't in namespace of user..."))
	}
